...
public class PainelDescarte extends JPanel{
	private JLabel jlDescarte, jlData, jlCEP;
	private JTextField jtfData, jtfCEP;
	private JButton jbCadastrar;
	private List<Coleta> coleta;
	
	public PainelDescarte(List<Coleta> coleta) {
		super();
		this.coleta = coleta;
		setSize(400, 400);
		setLayout(null);
	...
	private void iniciarComponentes() {
		jlDescarte = new JLabel("Descarte");
		...
		jlDescarte.setBounds(165, 10, 100, 20);
		...
		jbCadastrar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(!jtfData.getText().isEmpty() && !jtfCEP.getText().isEmpty()) {
					tipo = jlDescarte.getText();
					data = jtfData.getText();
					cep = jtfCEP.getText();	
					coleta.add(new ColetaDescarte(tipo, data, cep));
				} else {
